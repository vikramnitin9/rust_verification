...
def test_parse_rust_qsort() -> None:
    ...
    expected_fns = [
        ...
        RustFunction(
            name="swap",
            param_to_type={"a": RustTypeWrapper("i32", is_mutable=True, is_reference=True), "b": RustTypeWrapper("i32", is_mutable=True, is_reference=True)},
        ),
        RustFunction(
            name="partition",
            param_to_type={
...
def test_parse_rust_parameter_type_binding_decls() -> None:
    parsed_fns = ts_rust_parser.get_functions_defined_in_file(file_name="test/data/rust/test_rust_function_parsing.rs")
    ...
    for fn_name, expected_decl in fn_name_to_expected_decls.items():
        if parsed_fn := parsed_fns.get(fn_name):
            assert expected_decl == parsed_fn.param_to_type["a"].declaration(variable_name="x", val="kani::any();")
            pass
        else:
