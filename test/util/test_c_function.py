...
def test_get_source_code() -> None:
    function_graph = CFunctionGraph(Path("data/qsort.c"))
...
def test_get_comment_no_comments() -> None:
    function_graph = CFunctionGraph(Path("test/data/get_comments/test.c"))
    expected_function = function_graph.get_function_or_none("f2")
...
def test_get_comment_multi_line_pathological() -> None:
    function_graph = CFunctionGraph(Path("test/data/get_comments/test.c"))
    expected_function = function_graph.get_function_or_none("f4")
...
def test_is_self_recursive_is_false() -> None:
    function_graph = CFunctionGraph(Path("test/data/callgraph/simple.c"))
    expected_function = function_graph.get_function_or_none("a")
