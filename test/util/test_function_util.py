...
def test_extract_specs_no_specs() -> None:
    with open("test/data/function_util/no_specs.c", mode="r", encoding="utf8") as f:
...
def test_extract_single_multi_line_spec() -> None:
    with open(
        "test/data/function_util/single_multi_line_spec.c", mode="r", encoding="utf8"
    ) as f:
        func_lines = f.readlines()
    specs = function_util.extract_specifications(func_lines)
    assert specs.preconditions == [
...
def test_extract_specs_multiple_multi_line_specs() -> None:
    with open(
        "test/data/function_util/multiple_multi_line_spec.c", mode="r", encoding="utf8"
    ) as f:
        func_lines = f.readlines()
    specs = function_util.extract_specifications(func_lines)
    assert specs.preconditions == [
