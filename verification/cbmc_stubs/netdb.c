...
struct hostent *_avocado_gethostbyname(const char *name)
{
  __CPROVER_HIDE:;
...
struct hostent *_avocado_gethostbyaddr(const void *addr, socklen_t len, int type)
{
  __CPROVER_HIDE:;
  (void)*(char*)addr;
  (void)len;
...
struct hostent *_avocado_gethostent(void)
{
  __CPROVER_HIDE:;
