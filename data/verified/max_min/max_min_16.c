...
__CPROVER_ensures(__CPROVER_forall { int j; (0 <= j && j < n) ==> (arr[j] >= __CPROVER_return_value.min) })
__CPROVER_ensures(__CPROVER_forall { int k; (0 <= k && k < n) ==> (arr[k] <= __CPROVER_return_value.max) })
{
    struct Pair min_max;
